...
export const getOrderConfirmationFallback = (order: Order, configService: ConfigService) => {
  ...
  const subject = `Order Confirmed ✓ — #${id.slice(0, 8).toUpperCase()}`;
  const html = `
<!DOCTYPE html>
<html lang="en">
<head>
...
export const getNewOrderAlertFallback = (order: Order, configService: ConfigService) => {
  const { id, user, shippingName, totalAmount, shippingAddress, shippingCountry } = order;
  const frontendUrl = configService.get<string>('FRONTEND_URL') || 'http://localhost:3000';

  const subject = `New Order #${id.slice(0, 8).toUpperCase()} — $${Number(totalAmount).toFixed(2)} CAD`;
  const html = `
...
export const getStatusUpdateFallback = (order: Order, newStatus: string, configService: ConfigService) => {
...
export const getFraudCancellationFallback = (order: Order, configService: ConfigService) => {
  const { id, shippingName } = order;
  const frontendUrl = configService.get<string>('FRONTEND_URL') || 'http://localhost:3000';

  const subject = `Order Cancelled - Payment Method Mismatch`;
  const html = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#333">
